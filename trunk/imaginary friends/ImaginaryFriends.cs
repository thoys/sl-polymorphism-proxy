...
namespace imaginary_friends
{
    ...
    public class ImaginaryFriend : Avatar
    {
        ...
        ObjectUpdatePacket DefaultFriendUpdate()
        {
            ...
            {
                //Position
                Buffer.BlockCopy(LLVector3.Zero.GetBytes(), 0, objectdata, 16, 12);

                //Velocity
                Buffer.BlockCopy(LLVector3.Zero.GetBytes(), 0, objectdata, 28, 12);
                //Acceleration
                Buffer.BlockCopy(LLVector3.Zero.GetBytes(), 0, objectdata, 40, 12);
                //Rotation (theta)
                Buffer.BlockCopy(LLQuaternion.Identity.GetBytes(), 0, objectdata, 52, 12);
                //Angular Velocity (omega)
                Buffer.BlockCopy(LLVector3.Zero.GetBytes(), 0, objectdata, 64, 12);
            }
    ...
    public class ImaginaryFriends
    {
        ...
        Packet OnIncommingObjectUpdate(Packet packet, System.Net.IPEndPoint endp)
        {
            ...
            foreach (ObjectUpdatePacket.ObjectDataBlock b in ou.ObjectData)
            {
                ...
                {
                    if(!GlobalLocalIDs.ContainsKey(RegionHandle))
                        GlobalLocalIDs[RegionHandle] = new Dictionary<uint,LLUUID>();
